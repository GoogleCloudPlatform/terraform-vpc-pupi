...
echo 'Identifying kubernetes firewill rules in the VPC.'
FWRs="$(gcloud compute firewall-rules list \
         --project="$TF_VAR_pid" | grep INGRESS | \
         awk '!/^def/ {print $1}')"

if [[ "${#FWRs[@]}" -gt 0 ]]; then
  echo 'Deleteing firewall rules in the VPC.'
  gcloud compute firewall-rules delete "$FWRs" \
    --project="$TF_VAR_pid" \
    --quiet
else
    echo 'There are no FWRs in the VPC to delete.'
fi
